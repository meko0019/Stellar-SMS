...
import client
from tests.client.factories import create_sms, generate_otp, generate_stellar_address as gsa
...
def test_sms_parser(sms_body, expected, mocker):
    mocker.patch("client.transactions.tasks.create_tx.delay", lambda from_, to, amount, currency: ":)")
...
def test_sms_handler_pending_tx(mocker):
	...
	assert sms_handler(message) ==  "Incorrect password. Your transaction has been canceled."
